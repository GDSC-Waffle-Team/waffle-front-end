import React, { useState, useEffect, ChangeEvent, MouseEventHandler, useInsertionEffect } from 'react';
import axios from 'axios';
import { fine, list } from '.././constant'; // 쓰는 법을 모름
  ...
  useEffect(() => {
    const getInfo = async () => {
      const response = await axios
        .get('/api/member', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('loginToken')?.replace("'", '')}`,
          },
        })
        .then((response) => {
          ...
          <StyledTbody>
            {memberInfo.fines.map((fine) => (
              <StyledTr key={fine.id}>
                ...
                <StyledTd>{fine.type == '00' ? '지각' : fine.type == '01' ? '결석' : fine.type == '10' ? '미제출' : ''}</StyledTd>
